...
import json, os, threading, urllib.request, urllib.parse, io
from PIL import Image, ImageTk, ImageDraw
import model

TMDB_API_KEY = "cdba5cab59e7fb3b9d43a764ba734862"
...
def cargar_peliculas_por_generos(genres, n=10):
    """Fetch n random movies from TMDb matching any of the given genres."""
    ...
    url  = (f"https://api.themoviedb.org/3/discover/movie"
            f"?api_key={TMDB_API_KEY}&with_genres={genre_str}"
    ...
    try:
        ...
        peliculas = []
        for m in results[:n]:
            genre_names = [name for name, gid in TMDB_GENRE_IDS.items()
                           if gid in m.get("genre_ids", [])]
            peliculas.append({
                "title":       m["title"],
                "poster_path": m.get("poster_path", ""),
                "genres_clean": genre_names,
            })
        return peliculas
    except Exception as e:
        print(f"[TMDb] Error cargando películas: {e}")
        return []

def buscar_poster_por_titulo(title):
    """Search TMDb for a movie by title and return its poster_path."""
    try:
        query = urllib.parse.quote(title)
        url   = (f"https://api.themoviedb.org/3/search/movie"
                 f"?api_key={TMDB_API_KEY}&query={query}&language=en-US")
        data  = _tmdb_get(url)
        results = data.get("results", [])
        if results:
...
class MoviePickerDialog(tk.Toplevel):
    ...
    def _get_peliculas(self, genres):
        return cargar_peliculas_por_generos(genres, n=10)
    ...
    def _make_poster_card(self, parent, pelicula, idx):
        ...
        # Cargar poster desde TMDb API
        poster_path = pelicula.get("poster_path", "")
        def load(pp=poster_path, lbl=img_lbl, title=pelicula["title"]):
            path = pp if pp and str(pp) != "nan" else buscar_poster_por_titulo(title)
            if path:
                try:
                    req = urllib.request.Request(TMDB_BASE + path, headers={"User-Agent":"Mozilla/5.0"})
                    with urllib.request.urlopen(req, timeout=8) as r:
                        data = r.read()
            ...
            if self.winfo_exists():
                ...
                ImageDraw.Draw(ph2).text((10,80), title[:30], fill="#E5E7EB")
...
class DetailWindow(tk.Toplevel):
    ...
    def _favorites_row(self, parent, section_title, titles):
        ...
        for i, title in enumerate(titles[:5]):
            ...
            # Cargar poster desde TMDb API por título
            def load_poster(t=title, lbl=img_lbl, cw=CARD_W, ch=CARD_H):
                path = buscar_poster_por_titulo(t)
                if path:
                    try:
                        req = urllib.request.Request(TMDB_BASE + path, headers={"User-Agent":"Mozilla/5.0"})
                        with urllib.request.urlopen(req, timeout=8) as r:
    ...
    def _placeholder_row(self, parent, member):
        ...
        def cargar():
            try:
                recomendaciones = model.obtener_recomendaciones(member)
                if self.winfo_exists():
